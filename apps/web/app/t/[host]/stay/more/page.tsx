    ...
    <>
      ...
      {bookings?.data.map((b) => (
        <Row key={b.id} title={`${day(b.checkIn)} – ${day(b.checkOut)}`} sub={`${b.room?.split(' — ')[0]} · ${b.reference} · ${b.status.replace('_', ' ')}`}
          right={b.status === 'confirmed' ? <button className="t-link" onClick={async () => { try { const r = await guestApi<{ data: { fee: number } }>(`/portal/bookings/${b.id}/cancel`, { body: {} }); setMsg(r.data.fee ? `Cancelled. A fee of ${inr(r.data.fee)} applies under the rate’s policy.` : 'Cancelled — no fee.'); mutate(); reload(); } catch (e) { setMsg((e as Error).message); } }}>Cancel</button> : ' '} />
      ))}
