  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-[13px] text-muted">{forgot ? 'We’ll email you a link to choose a new password.' : 'Staff and platform accounts.'}</p>
      </div>
      {sent ? (
        ...
        <>
          ...
          <button className="btn btn-primary btn-lg w-full" disabled={busy}>{busy ? 'Please wait…' : forgot ? 'Send reset link' : 'Sign in'}</button>
        </>
      )}
